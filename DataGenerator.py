...
class DataGenerator2:
    ...
    def track_generator(self,n_tracks,sampling_rate=1,
        ...
        while len(self.T)<n_tracks:
            try:
                ...
                orig, dest = self.node_selector(date,**kwargs)
                track=nx.shortest_path(self.G, orig, dest, weight='perceived_length')
                self.T_as_path.append(self.nodes_to_edges(track))
    ...
    def load_map(self):
        G = ox.graph_from_place('Paris, France', network_type='bike',simplify=False,retain_all=False)

        preference_dict={'primary': 0.8,
                        'residential': 2,
                        'cycleway': 2.5,
                        'living_street': 2,
                        'tertiary': 1.5,
        ...
        def multiply_values(row):
            multiplier = preference_dict.get(row['highway'], 1)
            return row['length'] / multiplier

        gdf_edges['perceived_length'] = gdf_edges.apply(multiply_values, axis=1)
        
        self.G=ox.utils_graph.graph_from_gdfs(gdf_nodes, gdf_edges, graph_attrs=None)
